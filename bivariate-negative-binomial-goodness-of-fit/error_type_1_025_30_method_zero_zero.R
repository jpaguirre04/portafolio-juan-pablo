...
freczero<- function(a_0,a_1,tabla,N,v){
                        ...
                        if(g_2 < 0 || tabla[1,1]== 0 || abs(g_2 - min(a_0 ,a_1))< .4) g_2 <- 0
                        return(g_2)
                 }
########################################################################################################### 
...
for(k in 1:M){
    ...
    gamma_2<-  freczero(a_0=gamma_0,a_1=gamma_1,tabla,N,v)
     ...
     {
        B_obs<- B_est(a=gamma_0,b=gamma_1,c=gamma_2,datos_x=datosx,datos_y=datosy,N,v)}
        else{
               stop("Los parametros no cumplen los requisitos")
               on.exit
          }
    #print(B_obs)
                 for(j in 1:C){
                          ...
                          gammaboot_2<- freczero(a_0=gammaboot_0,a_1=gammaboot_1,tabla=tabla1,N,v)
