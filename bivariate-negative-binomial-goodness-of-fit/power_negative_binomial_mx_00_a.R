...
load("BBa.rda")
  for (m in 1:M){     
      datosx<- BBa[[m]]$x
      datosy<- BBa[[m]]$y
      tablabin<- table(datosx,datosy)
      gamma_0<-  mean(datosx)/v
      gamma_1<-  mean(datosy)/v
      gamma_2<-  m11(datos_a=datosx,datos_b=datosy,a_0=gamma_0,a_1=gamma_1,r=1,s=1,zaap=tablabin,n=N,v=v)
    ...
    for (b in 1:B){      
            ...
            gammaboot_2x<- est.maxima(datos_a=datosg$x,datos_b=datosg$y,a_0=gammaboot_0,a_1=gammaboot_1,a_2=gammaboot_2,N=N,v=v)
