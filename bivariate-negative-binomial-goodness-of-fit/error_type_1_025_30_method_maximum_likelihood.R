...
for(k in 1:M){
    ...
    datosx<-  Rucaa[[k]]$x
    datosy<-  Rucaa[[k]]$y
    tabla<-table(datosx,datosy)
    gamma_0<-  mean(datosx)/v
    gamma_1<-  mean(datosy)/v
    gamma_2<-  m11(datos_a=datosx,datos_b=datosy,a_0=gamma_0,a_1=gamma_1,r=1,s=1,zaap=tabla,n=N,v=v) 
    gamma_2x<- est.maxima(datos_a=datosx,datos_b=datosy,a_0=gamma_0,a_1=gamma_1,a_2=gamma_2,N,v)
     g <- 1+gamma_0+gamma_1-gamma_2x
     p1 <- (gamma_0-gamma_2x)/g
     p2 <- (gamma_1-gamma_2x)/g
     p3 <- gamma_2x /g
     EX <- (p1+p3)*v*g
     EY <- (p2+p3)*v*g
     VX <- EX*((p1+p3)*g+1)
     ...
     {
        B_obs<- B_est(a=gamma_0,b=gamma_1,c=gamma_2x,datos_x=datosx,datos_y=datosy,N,v)}
        else{
               stop("Los parametros no cumplen los requisitos")
               on.exit
          }
    #print(B_obs)
                 for(j in 1:C){
                          ...
                          gammaboot_2<- m11(datos_a=datosg$x,datos_b=datosg$y,a_0=gammaboot_0,a_1=gammaboot_1,r=1,s=1,zaap=tabla1,n=N,v=v)
                          gammaboot_2x<- est.maxima(datos_a=datosg$x,datos_b=datosg$y,a_0=gammaboot_0,a_1=gammaboot_1,a_2=gammaboot_2,N,v)
                          B_boot[j]<- B_est(a=gammaboot_0,b=gammaboot_1,c=gammaboot_2x,datos_x=datosg$x,datos_y=datosg$y,N,v)
                     }# fin de for anidado
       #print(B_boot)
