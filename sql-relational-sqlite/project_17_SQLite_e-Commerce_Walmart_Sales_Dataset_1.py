# -*- coding: utf-8 -*-
"""
Created on Tue May 20 19:28:32 2025
...
promedio de compras (`Purchase`) por cliente (`User_id`) 
para cada género (`Gender`)?
"""
# hacer de nuevo
query_2 = """
...
from (select
      ...
      product_category,
      media_compras,
      row_number() over(partition by gender order by media_compras desc) as ranking
...
"""
# 16.-
Crea una consulta que genere una secuencia de números desde 1 hasta el 
número máximo de categorías de productos (`Product_category`) que existen 
en la tabla `w`. La secuencia debe estar en una columna llamada 
`categoria_id`.
"""
...
with recursive secuencia as(
        ...
        categoria_id + 1
        from secuencia 
        where categoria_id < (select max(product_category)
                              from w)
        )
select
categoria_id
